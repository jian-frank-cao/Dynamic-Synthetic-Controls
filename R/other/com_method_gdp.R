...
compare_methods = function(data,
  ...
  # w/o TSDTW
  synth_origin = do_synth_90(df, "value_raw", dependent_id, start_time, n)
  if (plot_figures) {
  ...
  # w/ TSDTW
  synth_new = do_synth_90(df, "value_warped", dependent_id, start_time, n)
  if (plot_figures) {
...
length(which(result$improve>0))/nrow(result)
