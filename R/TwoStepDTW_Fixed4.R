...
second_dtw = function(x_post, x_pre,
  ...
  if (first_ind_left >= 0) {
    avg_weight[1] = avg_weight[1] + first_ind_left
  }else{
    cumsum_weight = cumsum(avg_weight)
    ind = min(which(cumsum_weight > abs(first_ind_left)))
    avg_weight[0:(ind - 1)] = 0
    avg_weight[ind] = cumsum_weight[ind] + first_ind_left
  }
