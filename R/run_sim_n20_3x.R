...
args.TFDTW = list(buffer = 3, match.method = "open.end",
                  dist.quant = 0.95, 
...
args.synth = list(predictors = NULL,
                  special.predictors = 
                    expression(list(list(dep.var, 7:9, c("mean")),
                                    list(dep.var, 4:6, c("mean")),
                                    list(dep.var, 1:3, c("mean")))),
                  time.predictors.prior = 1:9,
                  time.optimize.ssr = 1:9)

args.TFDTW.synth = list(start.time = 1, end.time = 20, treat.time = 10,
...
fig.big = df.gap %>%
  ...
  annotate("text", x = 5, y = 16, label = "bar(MSE)[SC]==32.73", parse = TRUE,
           col = "grey20", size = 5, fontface = "bold") +
  annotate("text", x = 5, y = 12, label = "bar(MSE)[DSC]==27.47", parse = TRUE,
           col = "grey20", size = 5, fontface = "bold") +
