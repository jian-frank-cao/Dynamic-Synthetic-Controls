...
result = result %>% mutate(improve = mse1_post - mse2_post)
boxplot(result$improve[  result$dependent!='Rhode Island'], outline = F)
abline(h = 0, lty = 5)

t.test(result$improve[result$year!=1992& result$dependent!='Rhode Island'])
t.test(result %>% filter(year == "1985"& result$dependent!='Rhode Island' ) %>% .[["improve"]])
t.test(result %>% filter(year == "1987" & result$dependent!='Rhode Island') %>% .[["improve"]])
t.test(result %>% filter(year == "1989"& result$dependent!='Rhode Island') %>% .[["ratio"]])
t.test(result %>% filter(year == "1992") %>% .[["ratio"]])
