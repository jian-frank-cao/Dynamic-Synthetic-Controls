...
# saveRDS(results, "./data/Figure_5_1_results.Rds")
...
# results = readRDS("./data/Figure_5_1_results.Rds")
...
df.target = data.frame(
  ...
  data.id = 1,
  grid.id = 1,
  value = results.target[[5]][[3]][["value"]],
  synth.sc = results.target[[5]][[3]][["synthetic"]],
  synth.dsc = results.target[[5]][[4]][["synthetic"]]
)
...
for (i in 1:nrow(df.mse)) {
  ...
  value.min = scales$value.min
  multiplier = scales$multiplier
  value = results[[data.id]][[1]][["results.TFDTW.synth"]][[3]][["value"]]
  synth.sc = results[[data.id]][[1]][["results.TFDTW.synth"]][[3]][["synthetic"]]
  synth.dsc = results[[data.id]][[1]][["results.TFDTW.synth"]][[4]][["synthetic"]]
